...
As a user with a limited budget, I want/need to see outfit suggestions that prioritise my existing wardrobe first and only recommend affordable add-ons when something essential is missing so that I don't feel pressured to spend money I don't have. 
As a user with low styling confidence, I want/need to receive simple, clear styling tips alongside each outfit combination so that I understand why pieces work together and feel more confident in my choice. 
...
As a user, I want to tag each clothing item with attributes like colour, type, and warmth level so that the system can match items intelligently to weather and occasion. 
As a user, I want to select which of my saved wardrobe items I have available today so that the system only builds outfits from clothes that are clean and accessible. 
...
Following that, outfit generation from wardrobe items becomes the main deliverable, though it can only be built once wardrobe selection exists. 
In fifth place come styling tips, which require low technical effort but have a high confidence impact for users like Riya who are unsure about fashion. 
Finally, affordable add-on suggestions are the lowest priority for the first sprint valuable, but secondary since Riya's primary goal is to work with what she already owns before considering any purchases. 
